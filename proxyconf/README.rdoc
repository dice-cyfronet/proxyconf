...
Returns list of registered workers. Response is in JSON format and it contains hash of hashes of arrays. Outer hash maps application_id, inner one - service name.

Example response:
  {"app_name":{"service_name":["10.0.0.2:9200"]}}
...
=== Registering workers: <tt>POST /workers/add/:application_id/:service_name</tt>
...
=== Unregistering workers: <tt>POST /workers/delete/:application_id/:service_name</tt>
...
See <tt>example/example_client.rb</tt>.
  ...
  proxyconf = RestClient::Resource.new('http://localhost:1234/')

  puts "Register two clients..."
  proxyconf["workers/add/app_name/service_name"].post workers: %w{10.0.0.1:9002 10.0.0.2:9200}
